...
function drawViz(data) {
  // Reset container on render updates
  document.body.innerHTML = '<div id="chart" class="chart-container"></div>';
  const container = document.getElementById('chart');

  // 1. Lock down the exact 5 items matching your dashboard layout
  const MASTER_CATEGORIES = [
    "AI in Education",
    "Early Learning",
    "Equity & Access",
    "K-12 & School Finance",
    "Postsecondary & Future of Work"
  ];

  // 2. Index the incoming native data records
  const dataMap = {};
  let maxVal = 1; // Default fallback to prevent divide-by-zero layout errors

  if (data.tables.DEFAULT) {
    data.tables.DEFAULT.forEach(row => {
      const categoryName = row.issueArea[0];
      const count = row.billCount[0];
      
      dataMap[categoryName] = {
        count: count,
        description: row.hoverText[0] || "No description provided.",
        rawRow: row
      };
  ...
  // 3. Render the locked visual rows matching your alignment specification
  MASTER_CATEGORIES.forEach(category => {
    const rowData = dataMap[category];
    const count = rowData ? rowData.count : 0;
    const desc = rowData ? rowData.description : "No bills available in this view.";
    const percentageWidth = (count / maxVal) * 100;
    ...
    barEl.className = 'bar';
    barEl.style.width = count > 0 ? `${percentageWidth}%` : '0px';
    barWrapper.appendChild(barEl);
    rowEl.appendChild(barWrapper);

    // Custom Formatted Tooltip Box HTML matching layout requirements
    ...
    `;
    rowEl.appendChild(tooltipEl);

    // 4. Native Looker Studio Interactions Framework Binding
    rowEl.addEventListener('click', () => {
      if (rowData && rowData.rawRow) {
        const filterData = data.interactions.crossFiltering;
        if (filterData.value === 'FILTER') {
          dscc.sendInteraction('crossFiltering', filterData.actionId, rowData.rawRow);
        }
      }
    });
...
// Attach component callback pipeline to real-time rendering framework
